...
import appInterface.DrawShapes;
...
public class DrawingCanvas extends PApplet{
	...
	private double lastX = -10, lastY = -10;
		...
		if (mousePressed == true) {
			if (insideCanvas(mouseX, mouseY)) {
				if (actualFlow1 > 0 || actualFlow2 > 0 || actualFlow3 > 0) {
					...
					addEllipseToArray(mouseX, mouseY, 0, 0, 0);  
				}
			}
		}

		try {
			if(isDrawingShape)Thread.sleep(200);
			Thread.sleep(50);
	...
	private void addLastPoint(ArrayList<RelativePoint> pointsLine) {
		if (!pointsLine.isEmpty()) {
			RelativePoint lastPoint = pointsLine.get(pointsLine.size()-1);
	...
	private void addEllipseToArray(double mX, double mY, int flow1, int flow2, int flow3) {
		...
		if (liveMode) {	// We check if we are in Live Mode
			...
			if ((!split) && (flavourChanged) && (!lastEllipse.isEmpty())){ // If we change the flavor, we add a last point with the new flavor 
				...
				lastEllipse.add(lastPoint);
				split = false;
				if (flavourChanged) flavourChanged = false;
			}
			lastEllipse.add(newPoint); 
